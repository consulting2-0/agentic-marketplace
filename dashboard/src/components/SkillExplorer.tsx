    ...
    <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            {isMarkdown && (
              <button onClick={() => { setSearchOpen(!searchOpen); if (!searchOpen) setTimeout(() => searchInputRef.current?.focus(), 50); }}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs transition-colors ${searchOpen ? 'bg-primary-50 text-primary-700' : 'text-text-tertiary hover:text-text-secondary hover:bg-surface-2'}`}
                title="Search in document (Cmd+F)">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
                </svg>
                <span className="hidden sm:inline">Search</span>
                <kbd className="hidden sm:inline text-[9px] px-1 py-0.5 rounded bg-surface-2 text-text-tertiary font-mono ml-1">
          ...
          <button onClick={handleCopy}
            className="text-[11px] px-2.5 py-1 rounded-md bg-surface-2 hover:bg-surface-3 text-text-tertiary hover:text-text-primary transition-colors">
            Copy
          </button>
        </div>
              ...
              <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-surface-2 border border-border rounded-lg shadow-xl overflow-hidden max-h-72 overflow-y-auto">
                {searchResults.map((result, i) => (
                  <button key={`${result.id}-${i}`} onClick={() => navigateToResult(result)} onMouseEnter={() => setSelectedIdx(i)}
                    className={`w-full text-left px-3 py-2.5 flex items-start gap-2.5 transition-colors ${i === selectedIdx ? 'bg-surface-3' : 'hover:bg-surface-2'}`}>
                    {result.type === 'heading' ? (
                    ...
                    {i === selectedIdx && <kbd className="text-[9px] px-1 py-0.5 rounded bg-surface-2 text-text-tertiary font-mono self-center shrink-0">Enter</kbd>}
                  </button>
                ))}
              </div>
            )}
                  ...
                  {currentHeadings.map((h) => (
                    <li key={h.id}>
                      <button onClick={() => scrollToHeading(h.id)}
                        ...
                        } ${activeHeading === h.id ? 'border-primary-700 text-primary-700' : 'border-transparent text-text-tertiary hover:text-text-secondary hover:border-border'}`}
                        title={h.text}>
  ...
  return (
    <div>
      <button onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-1 py-[3px] pr-2 hover:bg-surface-2 transition-colors group/f"
...
function FileNode({ node, depth, selected, onSelect }: { node: TreeNode; depth: number; selected: boolean; onSelect: (path: string) => void }) {
  ...
  return (
    <button onClick={() => onSelect(node.path)}
      className={`w-full flex items-center gap-1.5 py-[3px] pr-2 transition-colors ${selected ? 'bg-primary-50 text-primary-700' : 'hover:bg-surface-2 text-text-primary'}`}
...
function HighlightText({ text, query, className }: { text: string; query: string; className?: string }) {
  ...
  return (
    <span className={className}>
      {parts.map((p, i) => p.match ? <mark key={i} className="bg-primary-100 text-primary-900 rounded-sm px-px">{p.text}</mark> : <span key={i}>{p.text}</span>)}
    </span>
  );
}
